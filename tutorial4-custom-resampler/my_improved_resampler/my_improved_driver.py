...
class MyImprovedDriver(WEDriver):
        ...
        for bin in self.next_iter_binning:
            if len(bin) == 0:
                ...
                if np.any(init_check):
                    ...
                    maxval = np.max(pcoord)
                    minval = np.min(pcoord)
                    middle = maxval-minval/2

                    for iseg, segval in enumerate(pcoord):
                        progress[iseg] = 1/np.absolute(0-segval) + 1/np.absolute(middle-segval)

                    sorted_progress = np.argsort(-progress,axis=0)
                    top_half = sorted_progress[::-1][:int(nsegs/2)]
